...
async function init() {
  try {
    const stored = await browser.storage.local.get(['serverUrl', 'token']);
  ...
  try {
    ...
    if (currentTab) {
      ...
      if (currentTab.favIconUrl) {
        ...
        favicon.onerror = () => { favicon.src = 'icons/icon-48.svg'; };
      } else {
        favicon.src = 'icons/icon-48.svg';
      }
    }
  } catch (err) {
...
document.addEventListener('DOMContentLoaded', function() {
  $('login-form').addEventListener('submit', async function(e) {
    ...
    try {
      ...
      token = data.token;
      await browser.storage.local.set({ serverUrl: serverUrl, token: token });
      await showMainView();
